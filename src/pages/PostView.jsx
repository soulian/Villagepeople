...
export default function PostView() {
  ...
  useEffect(() => {
    const p = getPost(aptId, boardId, postId)
    setPost(p)
    if (p) {
      increaseViews(aptId, boardId, postId)
    }
  }, [aptId, boardId, postId])

  useEffect(() => {
    setComments(getComments(aptId, boardId, postId))
  }, [aptId, boardId, postId])
  ...
  const handleSubmitComment = (e) => {
    ...
    addComment(aptId, boardId, postId, { body: commentText.trim(), author })
    setComments(getComments(aptId, boardId, postId))
    setCommentText('')
  }
  ...
  if (!post) {
    return (
      <div className="hitel-card hitel-post-missing">
        <p>글이 없거나 삭제되었습니다.</p>
        <Link to={`/apt/${aptId}/board/${boardId}`}>[ 목록으로 ]</Link>
      </div>
    )
  }
  ...
  return (
    <div className="post-view hitel-card">
      <nav className="hitel-nav">
        <Link to={`/apt/${aptId}/board/${boardId}`}>◀ 목록</Link>
        <span># {boardName}</span>
      </nav>
